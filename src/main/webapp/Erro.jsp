<%-- 
    Document   : Erro
    Created on : 29/11/2016, 06:51:55
    Author     : Lucas
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>Erro!</title>
        
        <meta http-equiv="refresh" content="5; url=index.jsp">
        
        <!--Livraria JQuery-->
        <script src="https://code.jquery.com/jquery-3.1.1.js" integrity="sha256-16cdPddA6VdVInumRGo6IbivbERE8p7CQR3HzTBuELA=" crossorigin="anonymous"></script>

        <!-- Bootstrap CSS -->
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.0.0-alpha.5/css/bootstrap.min.css" integrity="sha256-mIfhv/h3MLq3WSiSlduuZO3saRNzzuf1LK8w3z3l3JY=" crossorigin="anonymous" />

        <!-- Bootstrap JavaScript -->
        <script src="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.0.0-alpha.5/js/bootstrap.min.js" integrity="sha256-j+XrvkQ4jJ99Hj0pJKPr6k0RCgxDDSTs3PBqLrX2EMc=" crossorigin="anonymous"></script>
        
    </head>
    <body>
        <div class="container">
            <div class="jumbotron jumbotron-fluid">
                <div class="container">
                    <h1 class="display-3">Erro!</h1>
                    <p class="lead">Você será redirecionado em breve!</p>
                    <div class="alert alert-danger" role="alert">
                        <h4 class="alert-heading">Falha ao comprar ingresso!</h4>
                        <p>Desculpe pelo transtorno, o lugar pode estar ocupado. Tente novamente.</p>
                        <p class="mb-0">"Você é o criador do seu próprio mérito."</p>
                    </div>
                </div>
            </div>
        </div>
    </body>
</html>
